...
describe('deriveTurnoStatus', () => {
  it('returns completado when an evolution is logged exactly on the scheduled date', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-07-01', scheduled_time: '10:00', status: 'programado' as const };
    const evolutions = [{ case_id: CASE_ID, created_at: '2026-07-01T12:00:00Z' }];
  ...
  it('returns completado when an evolution is logged 2 days after the scheduled date (within window)', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-07-10', scheduled_time: '10:00', status: 'programado' as const };
    const evolutions = [{ case_id: CASE_ID, created_at: '2026-07-12T09:00:00Z' }];
  ...
  it('returns completado when an evolution is logged exactly 3 days before the scheduled date (boundary, inclusive)', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-07-10', scheduled_time: '10:00', status: 'programado' as const };
    const evolutions = [{ case_id: CASE_ID, created_at: '2026-07-07T00:00:00' }];
  ...
  it('returns completado when an evolution is logged exactly 3 days after the scheduled date (boundary, inclusive)', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-07-10', scheduled_time: '10:00', status: 'programado' as const };
    const evolutions = [{ case_id: CASE_ID, created_at: '2026-07-13T00:00:00' }];
  ...
  it('does NOT complete when an evolution is 4 days after the scheduled date (outside window) and falls through to vencido', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-07-10', scheduled_time: '10:00', status: 'programado' as const };
    const evolutions = [{ case_id: CASE_ID, created_at: '2026-07-14T00:00:00' }];
  ...
  it('does NOT complete when an evolution is logged 10 days after the scheduled date, and stays programado if not yet past scheduled datetime', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-07-10', scheduled_time: '10:00', status: 'programado' as const };
    const evolutions = [{ case_id: CASE_ID, created_at: '2026-07-20T00:00:00' }];
  ...
  it('resolves to vencido when the scheduled date has passed and no qualifying evolution exists', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-06-01', scheduled_time: '09:00', status: 'programado' as const };
  ...
  it('resolves to programado when the scheduled date is in the future and no evolution exists', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-12-31', scheduled_time: '09:00', status: 'programado' as const };
    const today = new Date('2026-07-01T00:00:00');
  ...
  it('completado takes precedence over vencido when both conditions could apply', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-06-01', scheduled_time: '09:00', status: 'programado' as const };
    const evolutions = [{ case_id: CASE_ID, created_at: '2026-06-02T00:00:00' }];
    const today = new Date('2026-07-01T00:00:00');
    expect(deriveTurnoStatus(turno, evolutions, today)).toBe('completado');
  });

  it('a cancelado turno never becomes completado, even with a qualifying evolution', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-07-01', scheduled_time: '10:00', status: 'cancelado' as const };
    const evolutions = [{ case_id: CASE_ID, created_at: '2026-07-01T12:00:00Z' }];
    const today = new Date('2026-07-01T15:00:00Z');
  ...
  it('a cancelado turno never becomes vencido, even when the scheduled date is in the past', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-01-01', scheduled_time: '09:00', status: 'cancelado' as const };
    const today = new Date('2026-07-01T00:00:00');
    expect(deriveTurnoStatus(turno, [], today)).toBe('cancelado');
  });

  it('ignores evolutions belonging to a different case_id', () => {
    const turno = { case_id: CASE_ID, scheduled_date: '2026-07-01', scheduled_time: '10:00', status: 'programado' as const };
    const evolutions = [{ case_id: 'case-other', created_at: '2026-07-01T12:00:00Z' }];
...
describe('findTurnosToSupersede', () => {
  it('returns the id of an existing programado turno for the same case', () => {
    const existing = [
      { id: 't1', caseId: CASE_ID, status: 'programado' as const },
    ];
    expect(findTurnosToSupersede(existing, CASE_ID)).toEqual(['t1']);
  });

  it('returns the id of an existing vencido turno for the same case', () => {
    const existing = [
      { id: 't1', caseId: CASE_ID, status: 'vencido' as const },
    ];
    expect(findTurnosToSupersede(existing, CASE_ID)).toEqual(['t1']);
  });
  ...
  it('leaves completado turnos untouched', () => {
    const existing = [
      { id: 't1', caseId: CASE_ID, status: 'completado' as const },
    ];
    expect(findTurnosToSupersede(existing, CASE_ID)).toEqual([]);
  });

  it('leaves cancelado turnos untouched', () => {
    const existing = [
      { id: 't1', caseId: CASE_ID, status: 'cancelado' as const },
    ];
    expect(findTurnosToSupersede(existing, CASE_ID)).toEqual([]);
  });

  it('ignores turnos belonging to other cases', () => {
    const existing = [
      { id: 't1', caseId: 'case-other', status: 'programado' as const },
    ];
    expect(findTurnosToSupersede(existing, CASE_ID)).toEqual([]);
  });

  it('returns an empty array when there are no existing turnos', () => {
    expect(findTurnosToSupersede([], CASE_ID)).toEqual([]);
  });

  it('returns multiple ids when several unresolved turnos exist for the same case', () => {
    const existing = [
      { id: 't1', caseId: CASE_ID, status: 'programado' as const },
      { id: 't2', caseId: CASE_ID, status: 'vencido' as const },
      { id: 't3', caseId: CASE_ID, status: 'completado' as const },
      { id: 't4', caseId: 'case-other', status: 'programado' as const },
    ];
    expect(findTurnosToSupersede(existing, CASE_ID)).toEqual(['t1', 't2']);
  });
});
